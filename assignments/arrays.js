...
// console.log(`The last car in inventory is a ${lastCar.car_make} ${lastCar.car_model}`)
// console.log(`The last car in inventory is a ${inventory[inventory.length-1].car_make} ${inventory[inventory.length-1].car_model}`);

let lastCar2 = [];
for (let i=0; i < inventory.length; i++){
  lastCar2.push(inventory[i]);
}
console.log(`The last car in inventory is a ${lastCar2[0].car_make} ${lastCar2[0].car_model}`);

// alt - filter


const lastCarFilter = inventory.filter(car => car.id === inventory.length);
console.log(`The last car in inventory is a ${lastCarFilter[0].car_make} ${lastCarFilter[0].car_model}`);
...
for(let i=0; i < carYears.length; i++){
  if (carYears[i] < 2000){
    oldCarsS.push(carYears.car_year);
  }
}
console.log(`Spencer says there are ${oldCarsS.length} cars are older than 2000.` );
...
const oldCarsFilter = inventory.filter(car => car.car_year < 2000);
console.log(`${oldCarsFilter.length} cars are older than 2000.`)
